...
class ATPCore
  ...
  createDefaultCommandsFile: () ->
    if atom.config.get('terminalix.enableUserCommands')
      try
        initialContent = {
  ...
  init: () ->
    if not @state.opened
      ...
      try
        ...
        atom.notifications.addError 'terminalix: Could not load the config file.', {detail:e.message}
      if not @state.opened
        @createDefaultCommandsFile()
        @state.opened = true
  ...
  findUserCommandAction: (cmd) ->
    if not atom.config.get('terminalix.enableUserCommands')
      return null
